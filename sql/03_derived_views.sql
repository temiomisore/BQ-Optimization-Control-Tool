...
SELECT 
  COALESCE(h.director_name, 'Unassigned Director') AS director_name,
  COALESCE(h.director_email, 'unassigned@company.com') AS director_email,
  COALESCE(h.department, 'Cross-Functional / Core DW') AS department,
  j.project_id,
  ...
  ROUND(SUM(j.total_bytes_billed) / POW(1024, 4) * c.on_demand_usd_per_tib, 2) AS estimated_on_demand_spend_usd,
  ROUND(SUM(j.total_slot_ms) / (1000 * 3600), 1) AS total_slot_hours
FROM optimizer_ops.jobs_events j
CROSS JOIN optimizer_ops.v_config c
LEFT JOIN optimizer_ops.employee_hierarchy h
  ON LOWER(j.user_email) = LOWER(h.user_email)
WHERE j.job_type = 'QUERY'
GROUP BY 1, 2, 3, 4, c.on_demand_usd_per_tib;
...
WITH table_directors AS (
  SELECT 
    rt.project_id AS target_project,
    rt.dataset_id AS target_dataset,
    rt.table_id   AS target_table,
    COALESCE(h.director_name, 'Central Data Platform') AS director_name,
    COALESCE(h.department, 'Platform Infrastructure') AS department,
    COUNT(DISTINCT j.user_email) AS team_readers_count,
    COUNT(j.job_id) AS team_queries_count,
    ROUND(SUM(j.total_bytes_billed) / POW(1024, 3), 2) AS team_billed_gb
  FROM optimizer_ops.jobs_events j
  CROSS JOIN UNNEST(j.referenced_tables) rt
  LEFT JOIN optimizer_ops.employee_hierarchy h
    ON LOWER(j.user_email) = LOWER(h.user_email)
  GROUP BY 1, 2, 3, 4, 5
  QUALIFY ROW_NUMBER() OVER(
    PARTITION BY rt.project_id, rt.dataset_id, rt.table_id 
    ORDER BY COUNTIF(h.director_name IS NOT NULL) DESC, COUNT(j.job_id) DESC
  ) = 1
)
SELECT 
  r.change_set_id,
  ...
  r.state,
  COALESCE(td.director_name, 'Central Data Platform') AS director_name,
  COALESCE(td.department, 'Platform Infrastructure') AS department,
  COALESCE(td.team_readers_count, 0) AS team_readers_count,
  COALESCE(td.team_queries_count, 0) AS team_queries_count,
  COALESCE(td.team_billed_gb, 0.0) AS team_billed_gb,
