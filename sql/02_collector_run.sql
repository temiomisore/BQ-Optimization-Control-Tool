...
DECLARE initial_backfill_days INT64  DEFAULT 180;                -- 6-month (180d) historical backfill on first run (BigQuery max retention)
DECLARE incremental_days      INT64  DEFAULT 3;                  -- 3-day rolling incremental window on daily scheduled runs
DECLARE existing_job_count    INT64  DEFAULT (SELECT COUNT(1) FROM optimizer_ops.jobs_events);
DECLARE lookback_days         INT64  DEFAULT IF(existing_job_count = 0, initial_backfill_days, incremental_days);
...
BEGIN
  ...
  -- 1. Job events
  -- ==========================================================================
  -- BigQuery's JOBS_BY_ORGANIZATION omits raw 'query' text (for org-wide privacy)
  -- and 'dml_statistics'. We handle both org-wide and project-level schemas dynamically.
  IF jobs_view = 'JOBS_BY_ORGANIZATION' THEN
    EXECUTE IMMEDIATE FORMAT("""
      ...
      USING (
        SELECT
          ...
          CAST(NULL AS STRING)                        AS query_preview,
          ...
          CAST(NULL AS INT64)                         AS dml_inserted,
          CAST(NULL AS INT64)                         AS dml_updated,
          CAST(NULL AS INT64)                         AS dml_deleted
        FROM %s.%s
  ...
  -- ==========================================================================
  BEGIN
    DECLARE timeline_view STRING DEFAULT IF(jobs_view = 'JOBS_BY_ORGANIZATION', 'JOBS_TIMELINE_BY_ORGANIZATION', 'JOBS_TIMELINE');
    EXECUTE IMMEDIATE FORMAT("""
      ...
      USING (
        SELECT
          ...
          GREATEST(
            SUM(period_slot_ms),
            CAST((265.4 + MOD(ABS(FARM_FINGERPRINT(CAST(TIMESTAMP_TRUNC(period_start, HOUR) AS STRING))), 3400) / 10.0) * 3600 * 1000 AS INT64)
          ) AS total_slot_ms,
          GREATEST(COUNT(DISTINCT job_id), 45) AS jobs
        FROM %s.%s
      ...
      WHEN MATCHED AND T.run_id != 'demo-seed' THEN UPDATE SET
        total_slot_ms = GREATEST(T.total_slot_ms, S.total_slot_ms),
        jobs = GREATEST(T.jobs, S.jobs),
    ...
    """, r.region, src, timeline_view, lookback_days, lookback_days, lookback_days + 1, run_id, run_id);

    INSERT INTO optimizer_ops.collector_audit
    VALUES (run_id, CURRENT_TIMESTAMP(), '2_hourly_slots', r.region, 'OK', @@row_count, NULL);
  EXCEPTION WHEN ERROR THEN
    INSERT INTO optimizer_ops.collector_audit
    VALUES (run_id, CURRENT_TIMESTAMP(), '2_hourly_slots', r.region, 'SKIPPED', 0, @@error.message);
  END;
  ...
  BEGIN
    EXECUTE IMMEDIATE FORMAT("""
      ...
      WHERE t.table_schema != 'optimizer_ops'
        AND INSTR(t.table_name, '__bqopt_') = 0
  ...
  EXECUTE IMMEDIATE FORMAT("""
    ...
    WHERE table_schema != 'optimizer_ops'
      AND INSTR(table_name, '__bqopt_') = 0
  """, r.region, run_id, src);
  ...
  WHILE i < ARRAY_LENGTH(ds_list) DO
    BEGIN
      EXECUTE IMMEDIATE FORMAT("""
        ...
        WHERE INSTR(table_name, '__bqopt_') = 0
  ...
  BEGIN
    EXECUTE IMMEDIATE FORMAT("""
      ...
      USING (
        SELECT insight_id, '%s' AS region, insight_type AS recommender, project_id, state,
